...
- [New SqlGenerator implementations](../sql-generator) for Change -> SQL mappings that are wrong
- [New SnapshotGenerator implementations](../snapshot-generator) for snapshot logic that is wrong

The general pattern for both is to create a new class which returns a higher priority **_if and only if_** Liquibase is using the `Database` class you defined in [milestone 1](../milestone1-step1).
